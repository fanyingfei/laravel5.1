        ...
        if(!empty($pageList)){
            if($this->pageCount >1){
                if($this->jsFunction != 'href'){
                    if($this->hasPrePage){
                        $pageString = $pageString ."<a class='page-next' onclick='" .$this->jsFunction . "(" . ($this->pageNo-1) . ")'>上一页</a>";
                    }
                    ...
                    if($this->hasNextPage){
                        $pageString = $pageString ."<a class='page-next' onclick='" .$this->jsFunction . "(" . ($this->pageNo+1) . ")'>下一页</a>";
                    }
                }else{
                    $total_url = explode('/',$_SERVER['REQUEST_URI']);
                    $end_url = end($total_url);
                    if(is_numeric($end_url)) array_pop($total_url);
                    $url = implode('/',$total_url).'/';
                    if($this->hasPrePage){
                        $pageString = $pageString ."<a class='page-next' href='" .$url . ($this->pageNo-1) . "'>上一页</a>";
                    }
                    foreach ($pageList as $k=>$p){
                        if($this->pageNo == $p){
                            $pageString = $pageString ."<span class='page-cur'>" . $this->pageNo . "</span>";
                    ...
                    if($this->hasNextPage){
                        $pageString = $pageString ."<a class='page-next' href='" .$url . ($this->pageNo+1) . "'>下一页</a>";
                    }
                }
